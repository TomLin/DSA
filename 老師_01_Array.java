public class Array_Impl3 {
    ...
    public Array_Impl3(int size) {
        array = new Integer[size]; // 創建一個固定長度的array
        i_end = -1; // 這邊起始index用-1，因為我們index會從0開始
    }

    public void add_by_index(int i_add, int val) {
        if (i_end + 1 == array.length) expand_space(); // 檢查，是否array的空間已滿
        if (i_add > i_end + 1 || i_add < 0) return; // 檢查是否要插入的位置，不合理

        /** step01: move all value one slot after **/
        for (int i = i_end; i >= i_add ;i--) {
            array[i+1] = array[i]; // 將所有index之後的值，往後移一個
            array[i] = null;
        }
    ...
    public Integer search_by_index(int index) {
        if (index > i_end || index < 0) return null; //檢查index值合不合理(有沒有超出範圍)
        return array[index];
    }
    ...
    public static void main(String[] args) {
        ...
        /** search by value O(n) **/
        Integer val001 = myarray.search_by_value(2882); // 感謝學員 陳怡勳 建議反饋

        /** search by index O(1) **/
        Integer val002 = myarray.search_by_index(4);  // 感謝學員 陳怡勳 建議反饋
